...
export function SettingsPanel() {
  const { theme, accentColor, animations, toggleTheme, setAccentColor, setAnimations } = useTheme();
                ...
                <div className="grid grid-cols-6 gap-2">
                  {colorOptions.map((color) => (
                    <button
                      ...
                      className={cn(
                        "group relative h-10 rounded-md border-2 transition-all",
                      ...
                      title={color.label}
                    >
                      <div className={cn("w-full h-full rounded-sm", color.preview)} />
                      {accentColor === color.value && (
